...
pwd="$PWD"
trap "cd $pwd; ssh-agent -k &>/dev/null; tput cnorm; stty sane; rm -rf $HOME/tmp/list.txt 2>/dev/null" EXIT SIGINT
cd "$HOME" || exit 10
...
EXCLUDE_LIST=("sitecopy" "/.*" "/environment.*" "/.ssh" "/logs" "/tmp" "/php-session" "/php-upload")
...
getConfigFile() {
  ...
  if [[ "${#configArr[@]}" -gt 1 ]]; then
    ...
    for index in "${!configArr[@]}"; do
      printf "\033[1;32m%d\033[0m: \033[34m%s\033[0m\n" "$((index+1))" "${configArr[$index]/$homeDir\//}"
    done
...
transferFiles() {
  if [[ "$MANUAL_SOURCE" ]]; then
    rsync -ave "ssh -qi $SSH_PRIVKEY -p $SSHPORT" --exclude-from="$HOME/tmp/list.txt" "$SSHUSER@$SSHHOST:$SOURCE_DIRECTORY" "$DESTINATION_DIRECTORY" | pv -cli 0.1 -N"Files Transferred" >/dev/null &
  elif [[ "$DOCUMENT_ROOT" ]]; then
    # Rsync over the document root and place it in local document root
    rsync -ave "ssh -qi $SSH_PRIVKEY -p $SSHPORT" "$SSHUSER@$SSHHOST:$DOCUMENT_ROOT/" "$DESTINATION_DIRECTORY" | pv -cli 0.1 -N"Files Transferred" >/dev/null &
    # Rsync over non standard files and directories from remote user root
    [[ ! "$ONLY_DOCROOT" ]] && rsync -ae "ssh -qi $SSH_PRIVKEY -p $SSHPORT" --exclude-from="$HOME/tmp/list.txt" --exclude="/$(basename $DOCUMENT_ROOT)" "$SSHUSER@$SSHHOST:$SOURCE_DIRECTORY" "$HOME" >/dev/null &
  else
    rsync -ave "ssh -qi $SSH_PRIVKEY -p $SSHPORT" --exclude-from="$HOME/tmp/list.txt" "$SSHUSER@$SSHHOST:$SOURCE_DIRECTORY" "$DESTINATION_DIRECTORY" | pv -cli 0.1 -N"Files Transferred" >/dev/null &
  fi
}
...
# MAIN
mkdir -p "$HOME/tmp"
tr ' ' '\n' <<< "${EXCLUDE_LIST[@]}" > "$HOME/tmp/list.txt"
